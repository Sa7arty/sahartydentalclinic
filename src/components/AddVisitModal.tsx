...
/**
 * One modal for the whole "add a visit" flow — search/pick (or create) a patient,
 * then fill in the appointment fields — instead of navigating to a separate page.
 * Used from Schedule's "+ Add visit" button and from double-clicking a grid slot
 * (which passes `prefillScheduledAt` so the date/time field is already filled in).
 */
export default function AddVisitModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="max-h-[90vh] w-full max-w-lg space-y-4 overflow-y-auto rounded-xl bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-navy-900">Add visit</h2>
          <button onClick={onClose} className="text-sm text-navy-700 hover:underline">
            Cancel
          </button>
        </div>
          ...
          <>
            {!selectedPatient && (
              <div className="space-y-4">
                <div>
                  <label className="mb-2 block text-sm font-medium text-navy-900">Find the patient</label>
                  <input
                    autoFocus
                    value={search}
                ...
                <div className="border-t border-slate-100 pt-4">
                  <button onClick={() => setShowNewPatientForm((s) => !s)} className="text-sm font-medium text-navy-700 hover:underline">
            ...
            {selectedPatient && (
              <div className="space-y-4">
                <div className="flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50 p-3">
                  <div>
                    <p className="text-xs text-slate-500">Scheduling for</p>
                    <p className="font-medium text-navy-900">{patientFullName(selectedPatient)}</p>
                    <p className="text-xs text-slate-400">
                ...
                <form onSubmit={handleScheduleVisit} className="grid gap-3 sm:grid-cols-2">
                  <div>
                  ...
                  <button
                    type="submit"
                    disabled={scheduling}
                    className="rounded-lg bg-gold-500 px-4 py-2 font-medium text-navy-950 hover:bg-gold-400 disabled:opacity-50 sm:col-span-2"
                  >
