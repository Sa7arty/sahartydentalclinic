...
type SideTab = 'cashflow' | 'income' | 'expenses'
type Period =
  | 'today'
  | 'yesterday'
  | 'this_week'
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-1 rounded-lg border border-slate-300 bg-white p-1 text-sm">
          {(['cashflow', 'income', 'expenses'] as SideTab[]).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          <form onSubmit={handleAddExpense} className="grid gap-3 rounded-xl border border-slate-200 bg-white p-4 sm:grid-cols-2">
            ...
            <input name="category" placeholder="Category (optional)" className="rounded-lg border border-slate-300 px-3 py-2 text-sm" />
              ...
              editingExpenseId === e.id ? (
                <form key={e.id} onSubmit={(ev) => handleSaveExpenseEdit(ev, e)} className="space-y-2 border-b border-slate-100 px-4 py-3 last:border-0">
                  ...
                  <div className="grid gap-2 sm:grid-cols-2">
                    ...
                    <input name="category" defaultValue={e.category ?? ''} placeholder="Category" className="rounded-lg border border-slate-300 px-3 py-2 text-sm" />
